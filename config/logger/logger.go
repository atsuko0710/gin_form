...
func Init() (err error) {
	...
	if viper.GetString("mode") == "dev" {  // 开发模式，日志输出到终端
...
// getEncoder 获取编码器（日志格式）
func getEncoder() zapcore.Encoder {
	encoderConfig := zap.NewProductionEncoderConfig()  // 获取预先设置的编码器
...
func getLogWriter() zapcore.WriteSyncer {
	// 加入Lumberjack支持,实现切割归档日志文件
	lumberjackLogger := &lumberjack.Logger{
		Filename: viper.GetString("log.filename"),  // 日志文件的位置
		MaxSize: viper.GetInt("log.max_size"), // 在进行切割之前，日志文件的最大大小
